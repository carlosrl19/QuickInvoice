...
<!-- PDF Viewer Modal 3 -->
<div class="modal fade modal-blur" id="loan_account_statement{{ $loan->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
    role="dialog" aria-labelledby="pdfPaymentHistoryLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <p class="modal-title fw-bold" id="pdfPaymentHistoryLabel">Estado de cuenta #{{ $loan->loan_code_number }}</p>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <iframe id="loan_account_statement_iframe{{ $loan->id }}" style="width:100%; height: 55rem;" src=""></iframe>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // Cargar PDF en el modal de Estado de cuenta
        $('#loan_account_statement{{ $loan->id }}').on('show.bs.modal', function(event) {
            var url = '{{ route("loans.loan_account_statement_report", $loan->id) }}';
            $(this).find('#loan_account_statement_iframe{{ $loan->id }}').attr('src', url);
        });

        // Limpiar URL al cerrar el modal de Estado de cuenta
        $('#loan_account_statement{{ $loan->id }}').on('hidden.bs.modal', function() {
            $(this).find('#loan_account_statement_iframe{{ $loan->id }}').attr('src', '');
        });
    });
</script>